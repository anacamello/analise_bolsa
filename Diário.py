...
def cria_dados_acao_tabela (codigo_acao, dataInicial, dataFinal, tipoAtivo):
    ...
    if(tipoAtivo == "cripto"):
        ...
        if len(dados_acao_tabela) > 0:
            ...
            dados_acao_tabela.drop(["tick_volume", "spread"], axis = 1, inplace = True)
...
def formata_tabela_relatorio(tabela_relatorio):
    ...
    #tabela_relatorio['Preço de Entrada'] = pd.Series(["R$ {0: _.2f}".format(val) for val in tabela_relatorio['Preço de Entrada']], index = tabela_relatorio.index)
    #tabela_relatorio['Preço de Entrada'] = tabela_relatorio['Preço de Entrada'].astype(str)
    #tabela_relatorio['Preço de Entrada'] = tabela_relatorio['Preço de Entrada'].str.replace('.', ',')
    #tabela_relatorio['Preço de Entrada'] = tabela_relatorio['Preço de Entrada'].str.replace('_', '.')
...
dataFinalExibida = date.today() - timedelta(1)
